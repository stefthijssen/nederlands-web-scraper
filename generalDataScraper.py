...
START_URL = "https://dmoz-odp.org"
WORLD_URL = "https://dmoz-odp.org/World"
...
class DataSpider(scrapy.Spider):
    ...
    def parse(self, response):
        if not response.url.startswith(START_URL):
            ...
            odpHrefs = list(filter(lambda x : x.startswith(START_URL) and not x.startswith(WORLD_URL), hrefs))
            documentHrefs = response.css(DOCUMENT_HREF_SELECTOR).extract()

            for href in documentHrefs:
                yield scrapy.Request(href, callback=self.parse)

            for href in odpHrefs:
                yield scrapy.Request(href, callback=self.parse)